...
exports.login = async (userInfo) => {
    ...
    if(checkUserResponse.rows.length === 0) {
        const insertUserResponse = await pool.query('INSERT INTO users (id, email, display_name, display_picture) VALUES ($1, $2, $3, $4)', [userInfo.id, userInfo.email, userInfo.displayName, userInfo.photos[0].value]);
        if(insertUserResponse.rowCount === 0) throw new Error('Failed to insert user into database');
    }

    const token = jwt.sign({ id: userInfo.id }, process.env.JWT_SECRET, { expiresIn: '30d' });
